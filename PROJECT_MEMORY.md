...
**Hotfix (2026-05-05, on `master`, pushed):** ✅ Canvas drag-commit — node and comment drags now persist to the document via VueFlow's `@node-drag-stop` event, with multi-select drag wrapped in a single `history.transact` so undo reverts the whole gesture as one step. The previous predicate on `onNodesChange`'s `position` event (`change.position && !change.dragging`) could never match a drag commit because VueFlow's `updateNodePositions(items, false, false)` on dragstop emits with `change.position` unset, so dragged nodes lived only in VueFlow's internal store and were overwritten by `parseNode`'s `Object.assign` on the next reactive `:nodes` recompute (the typical trigger being selecting another node). The `onNodesChange` `position` branch is retained for arrow-key nudges (`useUpdateNodePositions` emits `changed=true, dragging=false`).

**Resumption point:** All work pushed; HEAD matches `origin/master`. Next session has no queued work.

Repo HEAD is `master` at `9fc286d` (in sync with `origin/master`). 47 Vitest files / 300 tests + 12 Playwright cases all green. `pnpm test/lint/typecheck/format:check/build/e2e` all exit 0. Working tree has two unstaged items unrelated to the hotfix: a `.gitignore` edit adding `phase1_graph.vizgraph.json` and a `.playwright-mcp/` scratch dir from in-session MCP browser testing.
